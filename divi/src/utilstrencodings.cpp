...
SecureString DecodeBase64Secure(const SecureString& input)
{
    ...
    // Preserve the original contract: input must be a whole number of base64 quads.
    if (input.size() % 4 != 0) {
    ...
    while (1) {
        ...
        case 0: // we have no bits and get 6
            ...
            break;
        case 1: // we have 6 bits and keep 4
            output += (char)((left << 2) | (dec >> 4));
            left = dec & 15;
            mode = 2;
            break;
        case 2: // we have 4 bits and get 6, we keep 2
            output += (char)((left << 4) | (dec >> 2));
            left = dec & 3;
            mode = 3;
            break;
        case 3: // we have 2 bits and get 6
            output += (char)((left << 6) | dec);
            mode = 0;
            break;
        }
    }
...
SecureString EncodeBase64Secure(const SecureString& input)
{
    ...
    int mode = 0, left = 0;
    const unsigned char* pch = (const unsigned char*)input.data();
    ...
    while (pch < pchEnd) {
        ...
        switch (mode) {
        case 0: // we have no bits
            output += pbase64[enc >> 2];
            left = (enc & 3) << 4;
            mode = 1;
            break;
        case 1: // we have two bits
            output += pbase64[left | (enc >> 4)];
            left = (enc & 15) << 2;
            mode = 2;
            break;
        case 2: // we have four bits
            output += pbase64[left | (enc >> 6)];
            output += pbase64[enc & 63];
            mode = 0;
            break;
        }
    }
